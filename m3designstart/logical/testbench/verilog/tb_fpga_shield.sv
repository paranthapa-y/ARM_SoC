...
// ARM stores the fractional counter in reversed bit order
assign mapped_cntr_f = {reg_baud_cntr_f[0],
                        reg_baud_cntr_f[1],
                        reg_baud_cntr_f[2],
                        reg_baud_cntr_f[3]};

// Integer reload condition
assign reload_i =
    baud_div_en &&
    (
        ((mapped_cntr_f >= reg_baud_div[3:0]) &&
         (reg_baud_cntr_i[15:1] == 15'd0))
      ||
...
begin
    ...
    begin
        if (reload_i)
            reg_baud_cntr_i <= reg_baud_div[19:4];
        else
            reg_baud_cntr_i <= reg_baud_cntr_i - 16'd1;
    end
end
...
begin
    ...
    begin
        if (reload_f)
            ...
            reg_baud_cntr_f <= reg_baud_cntr_f - 4'd1;
    end
end
...
reg [15:0] reg_baud_cntr_i1;
reg [3:0]  reg_baud_cntr_f1;
reg        reg_baud_tick1;

wire        baud_div_en1;
wire [3:0]  mapped_cntr_f1;
wire        reload_i1;
wire        reload_f1;

assign baud_div_en1 = (reg_ctrl1[1:0] != 2'b00);

// ARM stores the fractional counter in reversed bit order
assign mapped_cntr_f1 = {reg_baud_cntr_f1[0],
                        reg_baud_cntr_f1[1],
                        reg_baud_cntr_f1[2],
                        reg_baud_cntr_f1[3]};
...
assign reload_i1 =
    baud_div_en1 &&
    (
        ((mapped_cntr_f1 >= reg_baud_div1[3:0]) &&
         (reg_baud_cntr_i1[15:1] == 15'd0))
      ||
        (reg_baud_cntr_i1 == 16'd0)
    );
...
begin
    if (!n_rst)
        reg_baud_cntr_i1 <= 16'd0;
    else if (baud_div_en1)
    begin
        if (reload_i1)
            reg_baud_cntr_i1 <= reg_baud_div1[19:4];
        else
            reg_baud_cntr_i1 <= reg_baud_cntr_i1 - 16'd1;
    end
end
...
begin
    if (!n_rst)
        reg_baud_cntr_f1 <= 4'hF;
    else if (baud_div_en1)
    begin
        if (reload_f1)
            reg_baud_cntr_f1 <= 4'hF;
        else if (reload_i1)
            reg_baud_cntr_f1 <= reg_baud_cntr_f1 - 4'd1;
    end
end
...
begin
    if (!n_rst)
        reg_baud_tick1 <= 1'b0;
    else if (reload_i1 || reg_baud_tick1)
        reg_baud_tick1 <= reload_i1;
end
...
begin
    if (!n_rst) begin
        ...
        if (BAUD_COUNTER1 == 4'd15) begin
            BAUD_COUNTER1 <= 4'd0;
            BAUD_CLK1     <= ~BAUD_CLK1;
        end
        else begin
            BAUD_COUNTER1 <= BAUD_COUNTER1 + 1'b1;
        end
    end
end 
...
//ahb_if ahb_vif();
bind CORTEXM3INTEGRATIONDS ahb_bind u_ahb_bind
(
    //=========================================================
    // Clock / Reset
    //=========================================================
